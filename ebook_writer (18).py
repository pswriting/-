...
[현우 작가 스타일 목차 레퍼런스]
실제 베스트셀러 목차입니다. 이 스타일을 따라하세요:

챕터 제목 예시:
- "전자책으로 월 1,000만 원 벌기"
- "애플을 뛰어넘는 브랜딩 비법: NV 3원칙"
- "할머니도 베스트셀러로 만드는 'FOC 법칙'"
- "전자책 성공을 위한 10가지 시크릿 코드"
- "고객 구매율 5배 높이는 'P-10 법칙'"
...
- "그리지 못하는 언어는 쓰레기다"
- "가면을 쓰면 매출이 2배 증가한다"
- "500만 원 넘는 상품도 쉽게 파는 '3Y 전략'"
- "당신이 실패한 이유는 ○○○○ 때문이다"
- "시중에 나온 브랜딩 전략은 사기다"

[소제목 작성 규칙]
1. 구체적 숫자 필수 (300만원, 3억, 5배, 10가지)
2. 도발적 단정 ("~는 사기다", "~는 쓰레기다")
3. 비교 대조 ("A VS B")
4. 비밀/법칙 형태 ("'XX법칙'", "시크릿 코드")
5. ○○○○ 같은 빈칸으로 호기심 유발
6. 짧고 강렬하게 (15자 이내)
...
- "당신이 실패한 이유는 ○○○○ 때문이다"
- "500만 원 넘는 상품도 쉽게 파는 '3Y 전략'"
- "크몽 최상위 2% 전문가만 아는 비밀 기술"
- "제목과 첫 문장은 썸네일이다"
- "인포노마드에서 벗어나야 성공한다"
- "목차도 메이크업이 필요하다"

[작성 규칙]
1. 구체적 숫자 (300만원, 5배, 2%, 10가지)
2. 도발적 단정 (~는 사기다, ~는 쓰레기다)
3. 비교 대조 (A VS B)
4. 작은따옴표 법칙 ('XX법칙', 'XX전략')
5. ○○○○ 빈칸 호기심
6. 15자 이내로 짧게
...
def generate_subtopic_content(subtopic_title, chapter_title, questions, answers, topic, persona):
    ...
    prompt = f"""당신은 현우 작가입니다.
크몽 766건 거래, 전자책 베스트셀러 저자입니다.
...
2. 숫자와 팩트
   - 구체적 숫자 필수 (52.3%, 2조원, 1900:5)
   - 출처 명시 가능하면 추가
   - "한국의 99.9% 기업이 중소기업입니다" 스타일
...
4. 호기심 유발
   - "'연봉 5,700만 원이면 나쁘지 않은데?' 다음에 나올 자료를 살펴보면 음흉한 실체가 드러납니다."
   - 다음 내용이 궁금하게 만들기
...
[미션]
위 인터뷰 내용으로 '{subtopic_title}' 본문을 작성하세요.
현우 작가처럼 자신감 있고, 팩트 기반으로, 읽기 쉽게."""
    return ask_ai("현우 작가", prompt, temperature=0.8)
...
- 감정선 살리기"""
    ...
    prompt = f"""다음 글을 현우 작가 스타일로 다듬어주세요.
                    ...
                    with st.spinner("설계 중..."):
                        ...
                        for line in lines:
                            ...
                            is_chapter = (
                                ...
                                (len(line) > 0 and line[0].isdigit() and ('.' in line[:5] or ':' in line[:5]))
                            )
            ...
            if st.button("✅ 목차 저장하기", key="save_manual_outline"):
                if manual_outline.strip():
                    ...
                    for line in lines:
                        ...
                        # 챕터 판별 (챕터, Chapter, 숫자로 시작, 또는 에필로그/프롤로그)
                        is_chapter = (
                            line.lower().startswith('챕터') or 
                            line.lower().startswith('chapter') or
                            line.lower().startswith('에필로그') or
                            line.lower().startswith('프롤로그') or
                            (len(line) > 0 and line[0].isdigit() and ('.' in line[:5] or ':' in line[:5] or '장' in line[:5]))
                        )
    ...
    with col2:
        ...
        if st.session_state['outline']:
            ...
            for i, chapter in enumerate(st.session_state['outline']):
                ...
                col_ch, col_info, col_del = st.columns([4, 1, 0.5])
                with col_ch:
                    edited = st.text_input(f"챕터 {i+1}", value=chapter, key=f"chapter_{i}")
                    edited_outline.append(edited)
                with col_info:
                    if subtopic_count > 0:
                        st.markdown(f"<span style='color: green; font-size: 0.85em;'>소제목 {subtopic_count}개</span>", unsafe_allow_html=True)
                    else:
                        st.markdown("<span style='color: #999; font-size: 0.85em;'>소제목 없음</span>", unsafe_allow_html=True)
                with col_del:
            ...
            if st.button("💾 저장하기", key="save_outline"):
                ...
                for ch in st.session_state['outline']:
                    ...
                    for line in lines:
                        ...
                        if found_chapter:
                            ...
                            if (line_stripped.lower().startswith('챕터') or 
                                ...
                                (len(line_stripped) > 0 and line_stripped[0].isdigit() and ('.' in line_stripped[:5] or ':' in line_stripped[:5]))):
                                break
                    ...
                    if ch not in st.session_state['chapters']:
                        ...
                        # 기존 데이터에 새 소제목 추가
                        existing = st.session_state['chapters'][ch].get('subtopics', [])
                        for st_name in subtopics:
                            if st_name not in existing:
                                existing.append(st_name)
                                st.session_state['chapters'][ch]['subtopic_data'][st_name] = {
                                    'questions': [], 'answers': [], 'content': ''
                                }
                        st.session_state['chapters'][ch]['subtopics'] = existing
...
with tabs[3]:
    ...
    if st.session_state['outline']:
        ...
        # 현재 챕터 정보 표시
        subtopic_count = len(chapter_data['subtopics'])
        if subtopic_count > 0:
            st.success(f"📌 이 챕터에 {subtopic_count}개의 소제목이 저장되어 있습니다.")
        ...
        col_auto_gen, col_manual_add = st.columns(2)
        ...
        with col_auto_gen:
            ...
            if st.button("✨ 소제목 자동 생성", key="gen_subtopics"):
        ...
        if chapter_data['subtopics']:
            ...
            st.info("소제목이 없습니다. 자동 생성 또는 직접 추가해주세요.")
