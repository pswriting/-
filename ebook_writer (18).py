...
정확히 4개 챕터, 각 챕터당 3개 소제목 (총 12개)을 만들어주세요.

[★★★ 글자 수 제한 - 반드시 지키세요 ★★★]

챕터 제목: 최대 20자 (콜론 뒤 제목만)
소제목: 최대 15자

예시:
- "챕터1: 월급쟁이의 배신" (9자) ✅
- "챕터1: 지금 시작하지 않으면 평생 후회합니다" (20자 초과) ❌
...
- [15자 이내 소제목]
- [15자 이내 소제목]"""
    return ask_ai("베스트셀러 기획자", prompt, temperature=0.9)
...
def regenerate_chapter_outline(chapter_number, topic, persona, existing_chapters):
    ...
    emotion = chapter_emotions.get(chapter_number, "호기심")
...
def generate_subtopic_content(subtopic_title, chapter_title, questions, answers, topic, persona):
    ...
    prompt = f"""당신은 자청(역행자) 수준의 베스트셀러 작가입니다.
...
⚠️ 오직 '{subtopic_title}'에 대한 본문만 작성하세요.
...
4. 구체적 숫자와 상황
   - "열심히 했다" → "새벽 5시에 일어나 3시간 작업했습니다"
   - "많이 벌었다" → "월 320만 원을 벌었습니다"
   - "오래 걸렸다" → "정확히 47일이 걸렸습니다"

[★★★ 쉼표 사용 규칙 ★★★]

쉼표를 최소화하세요. 문장당 쉼표 1개 이하.

절대 금지:
- 주어 뒤 쉼표: "저는, 그때" (X) → "저는 그때" (O)
- "제가 생각하기에, 많은" (X) → "많은 직장인들이" (O)
- "사실, 이것은" (X) → "사실 이것은" (O)
- "그래서, 저는" (X) → "그래서 저는" (O)
- "하지만, 문제는" (X) → "하지만 문제는" (O)

쉼표가 2개 이상인 문장은 두 문장으로 나누세요.
...
[올바른 예시]
"저는 처음 3개월 동안 단 한 건도 팔지 못했습니다. 매일 새벽까지 글을 썼는데 반응은 제로였습니다. 뭐가 문제인지 몰랐습니다.

그러다 한 가지를 바꿨습니다. 제목에서 '방법'이라는 단어를 빼고 구체적인 숫자를 넣었습니다. 다음 주에 첫 판매가 일어났습니다."

[분량] 1500~2000자"""
    return ask_ai("베스트셀러 작가", prompt, temperature=0.7)


def refine_content(content, style="친근한"):
    style_guide = {
        "친근한": "자신감 있는 단정, 구체적 숫자와 팩트",
        "전문적": "데이터 강조, 논리적 전개",
        "직설적": "핵심만 간결하게, 군더더기 제로",
        "스토리텔링": "구체적 장면 묘사, 감정선"
    }
...
[★★★ 쉼표 규칙 ★★★]
쉼표를 대폭 줄이세요. 문장당 쉼표 1개 이하.

수정 필수:
- "저는, 그때" → "저는 그때"
- "제가 생각하기에, 많은" → "많은 직장인들이"
- "사실, 이것은" → "사실 이것은"
- "그래서, 저는" → "그래서 저는"
- "하지만, 문제는" → "하지만 문제는"

쉼표 2개 이상인 문장 → 두 문장으로 분리

[★★★ 클리셰/뻔한 표현 수정 ★★★]
다음 표현은 모두 삭제하거나 구체적 묘사로 교체:
- "마음은 콩밭에 가 있었다" → 삭제
- "눈앞이 캄캄했다" → 구체적 상황으로
- "가슴이 벅차올랐다" → 삭제
- "머리를 한 대 맞은 기분" → 삭제
...
[스타일] {style_guide.get(style, "담백하게")}

다듬어진 글만 출력하세요."""
    return ask_ai("에디터", prompt, temperature=0.6)
...
def clean_content_for_display(content, subtopic_title=None, chapter_title=None):
    ...
    # 추가: 외국어 문자 제거 (러시아어, 아랍어, 일본어 히라가나/가타카나 등)
    ...
    content = re.sub(r'[\u3040-\u309F]', '', content)
    # 일본어 가타카나: U+30A0 ~ U+30FF
    content = re.sub(r'[\u30A0-\u30FF]', '', content)
    # 태국어: U+0E00 ~ U+0E7F
    content = re.sub(r'[\u0E00-\u0E7F]', '', content)
