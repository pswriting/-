...
def generate_subtopic_content(subtopic_title, chapter_title, questions, answers, topic, persona):
    ...
    prompt = f"""당신은 20년 경력의 베스트셀러 작가입니다.
『역행자』, 『부의 추월차선』, 『돈의 속성』 같은 책을 집필한 수준의 글을 씁니다.
...
5. 구조: 자연스러운 흐름
   - 도입(상황/문제) → 전개(과정/시도) → 통찰(깨달음) → 마무리(적용점)
   - 불릿포인트, 번호 목록 사용 금지
   - 굵은 글씨, 밑줄 등 강조 표시 금지
   - 자연스러운 문장으로만 구성
...
- **굵은 글씨**, _기울임_ 등 마크다운
- "그래서 결론은", "정리하자면"
- "~해야 합니다", "~하셔야 합니다" 과도한 당위성

[분량]
1200~1800자

[미션]
위 인터뷰 내용을 바탕으로 '{subtopic_title}'에 대해 깊이 있고 몰입감 있는 글을 작성하세요.
한 편의 에세이처럼 자연스럽게 흘러가는 글이어야 합니다.
독자가 읽다가 "맞아, 맞아" 하며 공감하고, 끝까지 읽게 만드세요."""
...
- 구체적 숫자와 데이터로 신뢰 구축
- 차분하고 논리적인 전개
- "~라고 말할 수 있습니다", "~한 이유가 있습니다" """,
        
        "직설적": """엠제이 드마코(부의 추월차선) 스타일
- 핵심만 간결하게, 군더더기 제로
- 직설적이지만 존중하는 표현
- "이것만은 꼭 기억하세요", "흔히 하는 실수입니다"
- 독자의 안일함을 깨우면서도 응원하는 톤""",
        
        "스토리텔링": """에세이/스토리 중심 스타일
- 모든 내용을 구체적 장면으로 전달
- 시간, 장소, 감정을 생생하게 묘사
- "그날 저는...", "그때 깨달았습니다..."
- 독자가 함께 경험하는 듯한 몰입감"""
...
한 편의 에세이처럼 자연스럽게 흘러가는 글로 만들어주세요.
