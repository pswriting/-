...
# ==========================================
# 🔥 핵심 개선: 목차 생성 함수 (쉽고 매력적인 목차)
# ==========================================
def generate_outline(topic, persona, pain_points):
    prompt = f"""# Role
당신은 어려운 지식도 초등학생이 이해할 수 있게 풀어내는 베스트셀러 전자책 기획자입니다.
철학: "어려우면 팔리지 않는다"
목표: 목차만 읽어도 "아, 이제 방향은 잡혔다"는 느낌
...
## PART 1. 고정관념 깨기
- 어렵게 생각했던 문제가 사실은 쉬운 문제였음을 밝힘
- 복잡한 건 다 필요 없고, 핵심은 딱 하나
- "왜 지금까지 안 됐는지"를 먼저 드러냄
- 예: "복잡한 건 다 버리세요", "딱 하나만 알면 됩니다"

## PART 2. 누구나 따라 하는 공식
- 방법을 공식, 템플릿, 체크리스트처럼 쉬운 도구로 소개
- 따라 하는 데 걸리는 시간이 짧음을 강조
- "이건 나도 10분이면 하겠는데?" 느낌
- 예: "자기 전 10분", "그냥 보고 따라 하세요"

## PART 3. 오늘 당장 달라지는 것들
- 이 방법을 썼을 때 벌어질 구체적 변화
- 돈, 시간, 감정의 변화를 손에 잡힐 듯이
- 예: "내일 점심값은 여기서", "퇴근 후 2시간이 생깁니다"

## PART 4. 실패할 수 없는 구조
- 초보자가 두려워하는 실패, 손해를 어떻게 막는지
- "망하려고 해도 망할 수 없는 구조"
- 예: "잃어도 커피값", "최악에도 본전"
...
[쉬움 강조]
- "분석하는 법" (X) → "딱 3가지만 확인하세요" (O)
- "전략 수립" (X) → "아침 5분, 저녁 5분" (O)
- 독자가 "이건 나도 하겠는데?"라고 느끼게

[결과에 집중]
- 복잡한 원리 대신, 얻게 될 기분 좋은 결과를 제목에
- 여유 시간, 통장 잔고, 마음의 평화 등

[구체적 행동]
- 추상적 개념 대신 동사: 켜세요, 누르세요, 확인하세요
- 숫자로 구체화: 3개만, 10분, 21일

[톤]
- 이미 성공한 형/누나가 몰래 알려주는 느낌
- 차분하지만 단호하게, 과장 없이
- 판결문처럼 단정적으로
...
[평범한 표현]
- 서론, 개요, 이론, 기본, 방법, 정리, 결론, STEP, 가이드
- ~의 중요성, ~하는 법, 효과적인, 성공적인

[어려운 표현]
- 영어 약어 전면 금지 (KPI, ROI, R.P.M 등)
- 전문 용어 금지 (알고리즘, 메커니즘, 프레임워크)
- 대신 신호, 화면, 공식, 순서 등 쉬운 단어
...
## PART 2: [쉬운 공식을 암시하는 제목]
- [소제목]
- [소제목]
- [소제목]

## PART 3: [구체적 결과가 보이는 제목]
- [소제목]
- [소제목]
- [소제목]
...
- [소제목]
- [소제목]"""
    return ask_ai("베스트셀러 전자책 기획자", prompt, temperature=0.8)
...
def generate_subtopics(chapter_title, topic, persona, num_subtopics=3):
    prompt = f"""# Role
당신은 어려운 내용도 쉽게 풀어내는 베스트셀러 전자책 기획자입니다.
...
[쉬움 강조]
- "분석하는 법" (X) → "딱 3가지만 확인하세요" (O)
- 독자가 "이건 나도 10분이면 하겠는데?"라고 느끼게
- 15자 이내로 간결하게

[결과에 집중]
- 복잡한 원리 대신 얻게 될 결과를 제목에
- 시간, 돈, 감정의 변화가 보이게

[구체적 행동]
- 동사 사용: 켜세요, 누르세요, 확인하세요
- 숫자로 구체화: 3개만, 10분, 21일
...
# Output
'{chapter_title}' 챕터의 소제목 {num_subtopics}개.
번호와 소제목만:

1. [소제목]
2. [소제목]
3. [소제목]"""
    return ask_ai("전자책 기획자", prompt, temperature=0.8)


def regenerate_chapter_outline(chapter_number, topic, persona, existing_chapters):
    part_roles = {
        1: "고정관념 깨기 - 어렵게 생각했던 문제가 사실은 쉬운 문제였음을 밝힘",
        2: "누구나 따라 하는 공식 - 10분이면 할 수 있는 쉬운 방법 소개",
        3: "오늘 당장 달라지는 것들 - 구체적인 결과와 변화 묘사",
        4: "실패할 수 없는 구조 - 망하려고 해도 망할 수 없는 안전장치"
    }
    role = part_roles.get(chapter_number, "핵심 내용")
    prompt = f"""# Role
당신은 어려운 내용도 쉽게 풀어내는 베스트셀러 전자책 기획자입니다.
...
# Task
PART {chapter_number}를 새로 만드세요.
이 파트의 역할: {role}

# 원칙
- 쉬움 강조: "이건 나도 10분이면 하겠는데?" 느낌
- 결과에 집중: 얻게 될 기분 좋은 결과를 제목에
- 구체적 행동: 동사와 숫자 사용
- 톤: 형/누나가 알려주는 느낌, 차분하지만 단호
...
소제목만 출력 (번호, 기호 없이):"""
    result = ask_ai("전자책 기획자", prompt, temperature=0.8)
