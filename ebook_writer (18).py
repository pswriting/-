...
[제목 작성 원칙]
- 챕터 제목: 핵심 메시지를 담은 한 문장 (20자 이내)
- 소제목: 구체적이고 명확한 내용 (15자 이내)
- 숫자가 있으면 신뢰감 상승: "3단계", "21일", "월 500만원"

[절대 금지 - 유치한 표현]
- "야금야금", "술술", "쏙쏙", "팍팍", "쑥쑥" 등 의성어/의태어
- "비밀", "비법", "꿀팁", "대박", "핵심" 남발
- "~의 중요성", "~하는 방법", "효과적인"
- "이것", "그것", "이거" 반복
- 물음표(?)로 끝나는 질문형
...
def regenerate_chapter_outline(chapter_number, topic, persona, existing_chapters):
    chapter_roles = {
        1: "문제 인식 - 왜 이 책을 읽어야 하는가, 타겟의 문제 본질",
        2: "핵심 방법론 - 저자만의 독창적 기법과 실행 방법",
        3: "기대효과 - 적용 후 얻는 결과와 변화"
    }
    role = chapter_roles.get(chapter_number, "핵심 내용")
    prompt = f"""주제: {topic}
타겟: {persona}
