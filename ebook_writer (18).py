...
# ==========================================
# 🔥 핵심 개선: 목차 생성 함수 (살인적인 목차)
# ==========================================
def generate_outline(topic, persona, pain_points):
    prompt = f"""# Role
당신은 대한민국 상위 1% 전자책 매출을 기록하는 '초고수익 전자책 기획자'입니다.
당신의 목표는 단 하나: 독자가 목차를 읽는 순간, "이 내용은 돈을 주고서라도 지금 당장 확인해야 한다"는 강박을 느끼게 만드는 것입니다.
...
- 독자가 겪는 고통이 '노력 부족'이 아니라 '방법의 오류'임을 지적
- 기존 상식이나 경쟁자들의 조언이 왜 틀렸는지 폭로
- 독자가 "내 얘기잖아?"라고 소름 돋게 만들기

## PART 2. 비밀 공개 (The Mechanism)
- 당신만의 유일한 해결책(시스템/공식)을 소개
- 이 방법이 얼마나 빠르고, 쉽고, 재현 가능한지 어필
- 원리를 '도구'나 '공식'처럼 포장

## PART 3. 즉시 실행 (The Action)
- 당장 오늘부터 따라 할 수 있는 구체적 행동 지침
- 독자가 얻게 될 변화를 구체적 시나리오로 제시
- Copy & Paste 수준의 실행 가능성 강조

## PART 4. 확장과 유지 (The Scaling)
- 단순 문제 해결을 넘어선 장기적 비전
- 경제적/시간적 자유, 권위 획득 암시
- "이게 끝이 아니다"라는 확장 가능성
...
[심리 트릭]
- 부정 → 반전 → 해결 구조 사용
- 독자가 믿던 상식을 파괴하고, 새로운 방법론이 유일한 해결책임을 암시

[구체성]
- 숫자, 시간, 금액, 결과를 명시
- "열심히 하는 법" (X) → "하루 30분으로 월 100만 원 만드는 루틴" (O)
- "빠르게 성장" (X) → "21일 만에 첫 수익 발생" (O)
...
# 절대 금지 (Banned Words)
- 교과서 단어: "서론", "본론", "결론", "개념", "이해", "정의", "기초"
- 평범한 표현: "~의 중요성", "~하는 방법", "효과적인", "성공적인"
- 유치한 표현: "야금야금", "술술", "쏙쏙", "꿀팁", "대박"
- 애매한 표현: "이것", "그것", "여러 가지", "다양한"
- 물음표(?)로 끝나는 질문형 제목

# Output Format
설명 없이 아래 형식으로만 출력하세요.

## PART 1: [착각 붕괴 - 충격적인 제목]
- [소제목 1]
- [소제목 2]
- [소제목 3]

## PART 2: [비밀 공개 - 시스템/공식 이름]
- [소제목 1]
- [소제목 2]
- [소제목 3]
...
- [소제목 2]
- [소제목 3]"""
    return ask_ai("초고수익 전자책 기획자", prompt, temperature=0.85)
...
def generate_subtopics(chapter_title, topic, persona, num_subtopics=3):
    prompt = f"""# Role
당신은 전환율 10%의 카피라이터입니다.
소제목만 봐도 "이건 꼭 읽어야 해"라는 강박을 느끼게 만드세요.
...
# Output
'{chapter_title}' 챕터의 소제목 {num_subtopics}개를 만드세요.
번호와 소제목만 출력:

1. [소제목]
2. [소제목]
3. [소제목]"""
    return ask_ai("카피라이터", prompt, temperature=0.85)


def regenerate_chapter_outline(chapter_number, topic, persona, existing_chapters):
    part_roles = {
        1: "착각 붕괴 - 독자의 고통이 '노력 부족'이 아니라 '방법의 오류'임을 폭로. 기존 상식이 왜 틀렸는지 지적",
        2: "비밀 공개 - 당신만의 유일한 해결책(시스템/공식) 소개. 빠르고, 쉽고, 재현 가능함을 어필",
        3: "즉시 실행 - 당장 오늘부터 따라 할 수 있는 구체적 행동 지침. Copy & Paste 수준의 실행력",
        4: "확장과 유지 - 단순 문제 해결을 넘어선 장기적 비전. 경제적/시간적 자유 암시"
    }
...
- [소제목]
- [소제목]"""
    return ask_ai("전자책 기획자", prompt, temperature=0.85)
...
이 소제목만 봐도 "이건 꼭 읽어야 해"라는 강박을 느끼게.
